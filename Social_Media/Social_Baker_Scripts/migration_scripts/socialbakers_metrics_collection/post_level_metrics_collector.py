...
def read_sm_config(p_user_choice):
    ...
    global METRICS_TO_NORMALIZE, METRICS_TO_NORMALIZE_INTO, METRICS_TO_NORMALIZE_MAPPING
...
def get_brand_vertical_mapping(p_brand_vertical_mapping_url):
    ...
    brand_vertical_df["instagram_profile_id"] = brand_vertical_df["instagram_profile_id"].astype("Int64")
...
def date_limit_handler(p_start_date, p_end_date, p_user_choice, p_sync_type):
    ...
    if p_sync_type == 1 and user_choice in [0, 1, 2]:
...
def get_api_response(p_user_id, p_password, p_user_choice, p_start_date_arr, p_end_date_arr, p_profile_ids, p_post_level_metrics, p_limit, p_post_level_metrics_sort, p_sync_type):
...
def main():
    ...
    start_date_arr, end_date_arr = date_limit_handler(start_date, end_date, user_choice,SYNC_TYPE)
    ...
    if user_choice == 0:
        ...
        for i in range(1, 5):
            ...
            post_metrics_response = get_api_response(user_id, password, i, start_date_arr, end_date_arr, profile_ids, post_level_metrics, limit, post_level_metrics_sort,SYNC_TYPE)
        ...
        post_metrics_response = get_api_response(user_id, password, user_choice, start_date_arr, end_date_arr, profile_ids, post_level_metrics, limit, post_level_metrics_sort,SYNC_TYPE)
