...
def source_target_count_check():
    ...
    if (count_in_API_call == total_s3_count):
        ...
        print("Count in API Call month wise for the year:", start_date_1.year)
        for i in range(1, 13):
        ...
        print("fetching s3 count day wise for the mismatched month...")
        #s3_data_month_wise_count = s3_df.groupby(s3_df.created_time.dt.month)['post_id'].count()
        s3_data_day_wise_count = s3_df.groupby(s3_df.created_time.dt.month)['post_id'].count()
        # for i in mismatch_month:
        #     date_format = "%Y-%m-%d"
        #     d1 = dt.strptime(start_date_2.year+"-"+str(i)+"-01", date_format).date()
        #     last_date_of_month = calendar.monthrange(start_date_2.year, i)[1]
        #     d2 = dt.strptime(start_date_2.year+"-"+str(i)+"-"+str(last_date_of_month),date_format).date()
        #     d = d1
