...
def get_api_response(p_user_id, p_password, p_user_choice, p_start_date_arr, p_end_date_arr,p_next_date, p_profile_ids
        ...
        while True:
            ...
            if temp['success'] == True:
                if 'next' in list(temp['data'].keys()):
                    ...
                    if str(start_date) in temp_df_list:
                        break
                    if str(next_date) in temp_df_list:
                        break
...
def source_target_count_check():
    ...
    end_date_1 = end_date[0]
    print(type(start_date_1),type(end_date_1))
    next_date = end_date_1 + timedelta(days=1)
    print(type(next_date))

    post_metrics_response, total_val = get_api_response(user_id, password, user_choice, start_date, end_date,next_date,
    ...
    filtered_prev_year_df = df_api_response_1.loc[df_api_response_1.created_time.dt.date < start_date[0]]
    ...
    filtered_next_year_df = df_api_response_2.loc[df_api_response_2.created_time.dt.date < next_date]
    ...
    if (count_in_API_call == total_s3_count):
        ...
        for i in range(1, 13):
            start_date_2 = start_date[0]
            start_date_3 = dt.strptime(str(start_date_2.year) + "-" + str(i) + "-" + "01", '%Y-%m-%d').date()
            start_arr = [start_date_3]
            last_date_of_month = calendar.monthrange(start_date_2.year, i)[1]
            end_date_2 = dt.strptime(str(start_date_2.year) + "-" + str(i) + "-" + str(last_date_of_month),'%Y-%m-%d').date()
            end_arr = [end_date_2]
            next_date_2 = end_date_2 + timedelta(days=1)
            post_metrics_response1, total_val1 = get_api_response(user_id, password, user_choice, start_arr, end_arr,next_date_2,
            ...
            filtered_prev_month_df = df_api_response_3.loc[df_api_response_3['created_time'] < str(start_arr[0])]
            post_metrics_response, total_val = get_api_response(user_id, password, user_choice, start_arr, end_arr,next_date_2,
                                                                profile_ids, post_level_metrics, limit,
            ...
            filtered_next_month_df = df_api_response_4.loc[df_api_response_4['created_time'] < str(next_date_2)]
...
#            print(df_s3_month_day)
        ...
        for i in mismatch_month:
            ...
            while d < d2:
                ...
                next_date_3 = end_date_week[0]+timedelta(days=1)
                ...
                df_api_response_3 = pd.DataFrame(df_post_metric_response_3.api_response.apply(pd.Series))
                df_api_response_3['created_time'] = pd.to_datetime(df_api_response_3['created_time'], format='%Y-%m-%d')
                filtered_prev_month_df = df_api_response_3.loc[df_api_response_3['created_time'] < str(start_date_week[0])]
                post_metrics_response, total_val = get_api_response(user_id, password, user_choice, start_date_week, end_date_week,
                                                                    next_date_3,profile_ids, post_level_metrics, limit,
                                                                    post_level_metrics_sort, 1)
                df_post_metric_response_4 = pd.DataFrame(post_metrics_response)
                df_api_response_4 = pd.DataFrame(df_post_metric_response_4.api_response.apply(pd.Series))
                df_api_response_4['created_time'] = pd.to_datetime(df_api_response_4['created_time'], format='%Y-%m-%d')
