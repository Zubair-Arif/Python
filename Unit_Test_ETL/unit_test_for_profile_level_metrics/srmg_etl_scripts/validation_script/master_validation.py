"""
This document contains the python code that can validate the data count between source(SocialBakers) and
target(AWS S3 bucket). The API configuration and the AWS S3 configuration are available in the config.ini
file along with all other supporting parameters. Access Secrets are read from AWS SSM Parameter Store.
"""
...
from mail_sent import send_email
    ...
    for user_choice in range(1, 5):
        ...
        if SYNC_TYPE == 1:
            ...
            for year in range(start_year, end_year + 1):
                check = True
                start_date = dt.strptime(str(year) + "-01-01", "%Y-%m-%d").date()
                end_date = dt.strptime(str(year) + "-12-31", "%Y-%m-%d").date()
...
def main():
    ...
    mismatch_df = ga_validation()
    data_df = build_table(mismatch_df, "blue_light")
    send_email(data_df, log_list, from_email, to_email, cc_email, email_pass, log_link)
    return None
