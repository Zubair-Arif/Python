...
def main(event, context):
    ...
    for start,end in zip(start_date_arr, end_date_arr):
        ...
        for s,e in zip(start_month_arr, end_month_arr):
            ...
            if not c.append:
                wr.s3.to_parquet(df=df, path=aws.savepath.format(year=s[:4], month=s[5:7]), index=False)
            else:
                try:
                    master_df = wr.s3.read_parquet(path=aws.savepath.format(year=start[:4], month=s[5:7]))
                    df = pd.concat([master_df, df])
                except:
                    pass
                wr.s3.to_parquet(df=df, path=aws.savepath.format(year=start[:4], month=s[5:7]), index=False)
...
def parseConfig(filename):
    ...
    aws.savepath = config['Default']['savepath']
